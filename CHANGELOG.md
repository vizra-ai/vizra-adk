...
### Fixed
- Add `getConnectionName()` override to `AgentSession`, `AgentMessage`, and `AgentMemory` models so static Eloquent calls (e.g. `firstOrCreate`, `updateOrCreate` in `StateManager`) respect the configured `vizra-adk.database_connection` instead of falling back to the default connection. (#105, thanks @rohitp-bsf)
