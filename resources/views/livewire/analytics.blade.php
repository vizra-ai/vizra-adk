<div class="min-h-screen bg-gray-50">
    ...
    <div class="text-center mb-8">
        ...
        <p class="text-gray-600">Real-time insights into your AI agent performance and system health</p>
        @if($lastUpdated)
    ...
    <!-- Controls -->
    <div class="flex items-center justify-center space-x-4 mb-8 bg-white rounded-2xl p-4 border border-gray-200 shadow-sm max-w-md mx-auto">
        <!-- Auto Refresh Toggle -->
        <div class="flex items-center space-x-2">
            <label class="text-sm text-gray-600">Auto Refresh</label>
            <button 
                wire:click="toggleAutoRefresh"
                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 {{ $autoRefresh ? 'bg-blue-600' : 'bg-gray-200' }}"
            >
        ...
        <button 
            ...
            class="px-4 py-2 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-100 hover:border-gray-300 transition-colors flex items-center space-x-2"
        >
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="metric-card bg-white rounded-xl shadow-lg border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600">System Status</p>
                    <p class="text-2xl font-bold {{ $this->healthStatus === 'healthy' ? 'text-green-600' : ($this->healthStatus === 'warning' ? 'text-yellow-600' : 'text-red-600') }}">
        ...
        <div class="metric-card bg-white rounded-xl shadow-lg border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600">{{ $agentMetrics['active_sessions'] ?? 0 }}</p>
                </div>
        ...
        <div class="metric-card bg-white rounded-xl shadow-lg border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $agentMetrics['success_rate'] ?? 0 }}%</p>
                </div>
        ...
        <div class="metric-card bg-white rounded-xl shadow-lg border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-purple-600">{{ $agentMetrics['average_response_time'] ?? 0 }}ms</p>
                </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">{{ $agent['sessions'] }} sessions</p>
                    </div>
                    <div class="text-right">
                        <p class="text-sm text-gray-600">Avg Duration</p>
                        <p class="font-medium text-gray-900">{{ $agent['avg_duration'] }}s</p>
                    </div>
                </div>
                @empty
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{ $trend['messages'] }}</span>
                    </div>
                </div>
                @empty
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span class="text-sm text-gray-600">{{ $tool['count'] }} uses</span>
                </div>
                @empty
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <p class="font-semibold">{{ $toolUsageStats['tool_performance']['total_tool_calls'] ?? 0 }}</p>
                    </div>
                    <div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="bg-gray-50 rounded-lg p-3">
                    ...
                    <p class="text-xl font-bold text-gray-900">{{ $vectorMemoryAnalytics['total_documents'] ?? 0 }}</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-3">
                    <p class="text-sm text-gray-600">Storage</p>
                    <p class="text-xl font-bold text-gray-900">{{ $vectorMemoryAnalytics['storage_usage']['total_storage_mb'] ?? 0 }}MB</p>
                </div>
            </div>
            ...
            <div class="mt-4 space-y-2">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-medium">{{ $vectorMemoryAnalytics['search_performance']['average_search_time'] ?? 0 }}ms</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Cache Hit Rate</span>
                    <span class="font-medium text-green-600">{{ $vectorMemoryAnalytics['search_performance']['cache_hit_rate'] ?? 0 }}%</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Monthly Cost</span>
                    <span class="font-medium">${{ $vectorMemoryAnalytics['embedding_costs']['monthly_cost'] ?? 0 }}</span>
                </div>
            </div>
            @endif
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                        {{ ($systemHealth[$key]['status'] ?? 'unknown') === 'healthy' ? 'bg-green-100 text-green-800' : 
                           (($systemHealth[$key]['status'] ?? 'unknown') === 'warning' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
            ...
            <div class="space-y-3">
                <h4 class="font-medium text-gray-700">Memory Usage</h4>
                @if(!empty($systemHealth['memory_usage']))
                <div class="text-sm space-y-1">
                    <div class="flex justify-between">
                        <span class="text-gray-600">Current</span>
                        <span>{{ $systemHealth['memory_usage']['current_mb'] }}MB</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Peak</span>
                        <span>{{ $systemHealth['memory_usage']['peak_mb'] }}MB</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Limit</span>
                        <span>{{ $systemHealth['memory_usage']['limit_mb'] }}</span>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="space-y-3">
                <h4 class="font-medium text-gray-700">Response Times</h4>
                @if(!empty($systemHealth['response_times']))
                <div class="text-sm space-y-1">
                    <div class="flex justify-between">
                        <span class="text-gray-600">Database</span>
                        <span>{{ $systemHealth['response_times']['database'] }}ms</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Cache</span>
                        <span>{{ $systemHealth['response_times']['cache'] }}ms</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">API</span>
                        <span>{{ $systemHealth['response_times']['api'] }}ms</span>
                    </div>
                </div>
                @endif
            </div>
