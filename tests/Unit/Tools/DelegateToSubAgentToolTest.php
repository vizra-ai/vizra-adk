...
it('includes available sub-agents in description', function () {
    ...
    expect($definition['description'])->toContain('test_specialist_a, test_specialist_b');
});

it('executes delegation with complete context transfer', function () {
    $arguments = [
        'sub_agent_name' => 'test_specialist_a',
    ...
    expect($decodedResult['success'])->toBeTrue()
        ->and($decodedResult['sub_agent'])->toBe('test_specialist_a')
...
it('allows delegation within depth limits', function () {
    ...
    expect($decoded)->toBeArray()
        ...
        ->and($decoded['sub_agent'])->toBe('test_specialist_a');
});
...
it('increments delegation depth for sub-agent context', function () {
    ...
    // Replace the test_specialist_a with our mock
    ...
    $loadedSubAgents['test_specialist_a'] = $mockSubAgent;
...
{
    protected string $name = 'test-specialist-a';
...
class TestSpecialistBAgent extends BaseLlmAgent
{
